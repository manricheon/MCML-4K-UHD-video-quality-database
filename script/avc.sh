...
do
    if [ ${seq_num} = 010 ]
    then
        file_url=${root_url}"S"${seq_num:1}_${codec_type}".zip"
    else
        file_url=${root_url}"S"${seq_num}_${codec_type}".zip"
    fi
    wget -P ${dir_codec} ${file_url}
    unzip ${dir_codec}/"S"${seq_num}_${codec_type}".zip" -d ${dir_codec}/"S"${seq_num}
done



# BELOW script is not recommended beacuase decoding using ffmpeg has some problem
# for seq_num in 0{1..10}
# do
#     if [ ${seq_num} = 010 ]
#     then
...
#             if [ ${seq_num} = 010 ]
#             then
#                 file_path=${dir_codec}/"S"${seq_num:1}/"S"${seq_num}_A_${res_type}_${dst_type}
#             else
#                 file_path=${dir_codec}/"S"${seq_num}/"S"${seq_num}_A_${res_type}_${dst_type}
#             fi
#             echo ${file_path}

#             if [ ${res_type} == 01 ]
#             then
#                 ffmpeg -i ${file_path}.264 ${file_path}_temp.yuv
...
#             else
#                 ffmpeg -i ${file_path}.264 ${file_path}.yuv
#             fi
